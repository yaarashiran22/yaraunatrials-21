...
-- Fix vote column type in idea_votes to be text instead of boolean for consistency
ALTER TABLE public.idea_votes 
ALTER COLUMN vote TYPE TEXT USING (
  CASE 
    WHEN vote = true THEN 'agree'
    WHEN vote = false THEN 'disagree'
    ELSE vote::text
  END
);
