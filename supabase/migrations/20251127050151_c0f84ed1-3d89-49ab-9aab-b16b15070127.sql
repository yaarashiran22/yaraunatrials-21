...
BEGIN
  ...
  IF mood_filter = '*' THEN
    mood_filter := NULL;
  END IF;
  IF event_type_filter = '*' THEN
    event_type_filter := NULL;
  END IF;
  IF neighborhood_filter = '*' THEN
    neighborhood_filter := NULL;
  END IF;
  IF category_filter = '*' THEN
    category_filter := NULL;
  END IF;
  IF venue_name_filter = '*' THEN
    venue_name_filter := NULL;
  END IF;
      ...
      AND (
        ...
        OR (
          CASE
            ...
            -- Unrecognized format: include event
            ELSE true
          END
        )
      )
